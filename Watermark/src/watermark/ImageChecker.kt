...
object ImageChecker {
    ...
    fun checkIfNotLarger(larger: BufferedImage, smallerOrEqual: BufferedImage) {
        if (smallerOrEqual.width > larger.width || smallerOrEqual.height > larger.height) {
            val failMessage = "The watermark's dimensions are larger."
            throw InformAndTerminateException(failMessage)
        }
    }
}
